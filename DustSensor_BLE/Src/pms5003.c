...
void PMS5003_ReceiveStart_IT(UART_HandleTypeDef* handle){
	HAL_UART_Receive_IT(&huart1, PMS5003.uartBuf, 1);
}
/**
...
void PMS5003_ReceiveData_IT(UART_HandleTypeDef* handle){
	...
	// Start receiving PMS5003 data		
		HAL_UART_Receive_IT(handle, PMS5003.uartBuf, 31); //waiting for 31 byte rest
		PMS5003.state = PMS5003_RECEIVING; //state of process = RECEIVING
	}
	if (PMS5003.uartBuf[0] == PMS_START_BYTE_2 && PMS5003.state == PMS5003_RECEIVING) {
			PMS5003.frame = ((PMS5003.uartBuf[1] << 8) + PMS5003.uartBuf[2]); //get frame
			PMS5003.pm1p0 = ((PMS5003.uartBuf[3] << 8) + PMS5003.uartBuf[4]); //get pms1.0 data
			PMS5003.pm2p5 = ((PMS5003.uartBuf[5] << 8) + PMS5003.uartBuf[6]); //get pms2.5 data
			PMS5003.pm10 = ((PMS5003.uartBuf[7] << 8) + PMS5003.uartBuf[8]); //get pms10 data
			PMS5003.pm1p0_ATM = ((PMS5003.uartBuf[9] << 8) + PMS5003.uartBuf[10]); //get pms1.0 data in atmosphere
			PMS5003.pm2p5_ATM = ((PMS5003.uartBuf[11] << 8) + PMS5003.uartBuf[12]); //get pms2.5 data in atmosphere
			PMS5003.pm10_ATM = ((PMS5003.uartBuf[13] << 8) + PMS5003.uartBuf[14]); //get pms10 data in atmosphere
			PMS5003.checksum = ((PMS5003.uartBuf[29] << 8) + PMS5003.uartBuf[30]); //get checksum
			HAL_UART_Receive_IT(handle, PMS5003.uartBuf,1); //waiting for incoming new data
